...
//                        listeners.add(wrapAsSSE(sseActor));
                ...
                if (msg instanceof HttpStreamOpened)
                    p = wrapAsSSE(p);
                listeners.add(p);
                p.send(new WebDataMessage(self(), "IMPROVED: Welcome. " + listeners.size() + " listeners"));
            } // -------- WebSocket message received -------- 
            else if (message instanceof WebDataMessage) {
                WebDataMessage msg = (WebDataMessage) message;
                if (!msg.isBinary()) {
                    for (SendPort listener : listeners)
                        listener.send(new WebDataMessage(self(), "IMPROVED: local counter:" + i + " data:" + msg.getStringBody().toUpperCase()));
                }
            } // -------- Timeout -------- 
            else if (message == null) {
                for (SendPort listener : listeners)
                    listener.send(new WebDataMessage(self(), "IMPROVED: local counter:" + i + " no data. "));
            }
            i++;
